  ...
  return (
    <section id="destinations" className="py-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative overflow-hidden bg-noise">
      ...
      <div className="text-[10px] font-cinzel tracking-[0.3em] text-solara-gold/40 uppercase mb-4 text-center">
        [ MODULE 08 — REGIONAL SUMMER DESTINATION ARCHITECTURE ]
      </div>
      ...
      <div className="text-center max-w-3xl mx-auto mb-16">
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-solara-gold/15 text-solara-gold-light border border-solara-gold/30 text-xs font-semibold mb-4">
          ...
          <span>Micro-Market Destination Strategy</span>
        </div>
        ...
        {destinationsData.map((dest) => {
          ...
          return (
            <button
              ...
              className={`p-4 sm:p-5 rounded-2xl text-right transition-all border text-xs flex flex-col justify-between space-y-2 relative overflow-hidden ${
                isSelected
                  ? 'bg-solara-gold/20 border-solara-gold text-white shadow-luxury scale-102'
            ...
            >
              <div className="flex items-center justify-between w-full">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-xs sm:text-sm">{dest.nameAr.split('—')[0]}</span>
                </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 space-y-6 text-right">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-black/40 p-4 rounded-2xl border border-white/10 space-y-1">
                  <span className="text-[10px] text-solara-gold font-bold uppercase tracking-wider block">الأجواء والمزاج (Vibe)</span>
                  <p className="text-xs text-slate-200 leading-relaxed">{currentDest.vibeAr}</p>
                </div>

                <div className="bg-black/40 p-4 rounded-2xl border border-white/10 space-y-1">
                  <span className="text-[10px] text-solara-gold font-bold uppercase tracking-wider block">الجمهور المستهدف (Audience)</span>
                  <p className="text-xs text-slate-200 leading-relaxed">{currentDest.targetAudience}</p>
                </div>
              </div>

              {/* Product Specifications Required */}
              <div className="space-y-3">
                <span className="text-xs font-bold text-white block">مواصفات وتعديلات المنتج المصممة للوجهة:</span>
                <div className="space-y-2">
                  {currentDest.productRequirementsAr.map((req, idx) => (
                    <div key={idx} className="flex items-start gap-2.5 text-xs text-slate-200">
                      <CheckCircle2 className="w-4 h-4 text-solara-gold shrink-0 mt-0.5" />
                      <span>{req}</span>
                    </div>
                  ))}
                </div>
            ...
            {/* Right/Stats Column */}
            <div className="lg:col-span-5 space-y-6 text-right bg-black/50 p-6 sm:p-8 rounded-2xl border border-solara-gold/30">
              <div className="space-y-1">
                <span className="text-[10px] text-slate-400 font-mono uppercase block">Recommended Line</span>
                <h4 className="text-base font-bold text-solara-gold-light font-display">
                  {currentDest.recommendedCollection}
                </h4>
              </div>

              <div className="pt-4 border-t border-white/10 space-y-1">
                <span className="text-[10px] text-slate-400 font-mono uppercase block">Average Order Value (AOV)</span>
                <div className="text-xl font-bold text-white font-mono flex items-center gap-1.5">
                  <DollarSign className="w-5 h-5 text-emerald-400" />
                  <span>{currentDest.avgOrderValue}</span>
                </div>
              </div>

              {/* Color Palette Swatches */}
              <div className="pt-4 border-t border-white/10 space-y-2">
                <span className="text-[10px] text-slate-400 font-mono uppercase block">Palette Inspiration</span>
                <div className="flex items-center gap-2">
                  {currentDest.colorPalette.map((color, cIdx) => (
                    <div key={cIdx} className="flex flex-col items-center gap-1">
                      <div className="w-8 h-8 rounded-xl border border-white/30 shadow-md" style={{ backgroundColor: color }} />
